...
const SalesHistory = ({
  ...
  const cancelSale = (sale) => {
    if (
      !window.confirm(
        "¿Estás seguro de anular esta venta? Se descontará del dinero ganado y los productos/canastas volverán a tu stock disponible.",
      )
    ) {
      return;
    }
  ...
  const periodProfit = filteredSales.reduce(
    (acc, s) => acc + (Number(s.profit) || 0),
    0,
  );
  const periodMargin =
    periodRevenue > 0 ? (periodProfit / periodRevenue) * 100 : 0;
    ...
    filteredSales.forEach((sale) => {
      ...
      if (sale.items && sale.items.length > 0) {
        sale.items.forEach((item, index) => {
          ...
          const descuento =
            index === 0 && sale.discountApplied > 0 ? sale.discountApplied : 0;
          const ingresoFinal = subtotalBruto - descuento;
          const gananciaNeta =
            ingresoFinal - (item.cost || 0) * (item.quantity || 1);
    ...
    <div>
      ...
      >
        <div
          ...
          style={{ padding: "1.5rem", borderTop: "4px solid #60a5fa" }}
        >
          <p
            style={{
        ...
        <div
          className="glass-panel"
          style={{ padding: "1.5rem", borderTop: "4px solid #34d399" }}
        >
          <p
            style={{
              color: "var(--text-muted)",
          ...
          <h3 style={{ fontSize: "2rem", color: "var(--text-main)" }}>
            {formatCLP(periodRevenue)}
          </h3>
        </div>
        ...
        >
          ...
          <h3
            style={{
              fontSize: "2rem",
              color: periodProfit >= 0 ? "#10B981" : "var(--danger)",
            }}
          >
