...
const POS = ({
  ...
  const addToCart = (itemData, qty = 1) => {
    const existing = cart.find(
      (item) => item.id === itemData.id && item.type === itemData.type,
    );
    if (existing) {
      if (existing.quantity + qty > (itemData.stock || 0)) {
        setAlertPrompt({
          message: `No hay suficiente stock de ${itemData.name}. Máximo disponible: ${itemData.stock || 0}`,
          type: "error",
        });
        return;
      }
      ...
      if ((itemData.stock || 0) < qty) {
        setAlertPrompt({
          message: `No hay suficiente stock de ${itemData.name}`,
          type: "error",
        });
        return;
      }
      ...
      cart.map((item) => {
        if (item.id === id && item.type === type) {
          ...
          if (newQ > (sourceItem?.stock || 0)) {
            setAlertPrompt({
              message: `Stock insuficiente de ${item.name}`,
              type: "error",
            });
            return item;
          }
    ...
    cart.forEach((item) => {
      ...
      if (applicableOffers.length > 0) {
        ...
        if (offer.type === "bulk" && item.quantity >= offer.bulkMinQty) {
          ...
          itemTotal =
            sets * offer.nxmPay * item.sellPrice + remainder * item.sellPrice;
  ...
  const processCheckout = async () => {
    ...
    setProducts((prevProducts) => {
      const updatedProducts = prevProducts.map((p) => {
        const cartItem = cart.find(
          (item) =>
            item.id?.toString() === p.id?.toString() && item.type === "product",
        );
        if (cartItem) {
          return {
            ...p,
    ...
    setBaskets((prevBaskets) =>
      prevBaskets.map((b) => {
        const cartItem = cart.find(
          (item) =>
            item.id?.toString() === b.id?.toString() && item.type === "basket",
        );
        if (cartItem) {
          return {
            ...b,
            stock: Math.max(0, (b.stock || 0) - cartItem.quantity),
          };
        }
        return b;
      }),
    ...
    const newSale = {
      id: generateCode("VNT"),
      name:
        saleItems.length > 40 ? saleItems.substring(0, 40) + "..." : saleItems,
      seller: sellerName,
      revenue: total,
  ...
  useEffect(() => {
    const handleKeyDown = (e) => {
      ...
      if (checkoutStep === "payment") {
        if (e.key === "Escape") {
          ...
          setPaymentMethod((prev) =>
            prev === "Efectivo" ? "Transferencia" : "Efectivo",
          );
      ...
      if (e.key === "Enter") {
        ...
        if (searchTerm === "" && cart.length > 0) {
          ...
          if ((selectedItem.stock || 0) > 0) {
            ...
            setAlertPrompt({
              message: `No hay stock de ${selectedItem.name}`,
              type: "error",
            });
          }
        }
      ...
      if (
        ...
        document.activeElement !== rutInputRef.current &&
        !alertPrompt
      ) {
        searchInputRef.current?.focus();
      }
  ...
  return (
    <>
      ...
      <div
        style={{ display: "flex", gap: "2rem", height: "calc(100vh - 4rem)" }}
      >
        {/* Product List */}
        ...
        <div style={{ flex: 2, display: "flex", flexDirection: "column" }}>
          ...
          {keyboardMode && (
            ...
            >
              <span>
                <strong>Teclado:</strong> Escribe para buscar
              </span>
              <span>
                <strong>Tab/Flechas:</strong> Seleccionar
              </span>
              <span>
                <strong>Enter:</strong> Preguntar cantidad
              </span>
              <span>
                <strong>Enter x2 (Buscador vacío):</strong> Cobrar
              </span>
              <span>
                <strong>+:</strong> Cobrar rápido
              </span>
            </div>
          )}
              ...
              style={{
                ...
                background:
                  selectedCategory === "all"
                    ...
                    : "var(--panel-alt)",
                color:
                  selectedCategory === "all" ? "white" : "var(--text-main)",
          ...
          <div style={{ overflowY: "auto", flex: 1, paddingRight: "0.5rem" }}>
            {alertPrompt && (
              <div
                style={{
                  background:
                    alertPrompt.type === "success"
                      ? "var(--panel-alt)"
                      : "var(--panel-alt-2)",
                  padding: "2rem",
                  borderRadius: "12px",
                  border: `2px solid ${alertPrompt.type === "success" ? "#10B981" : "var(--danger)"}`,
                  marginBottom: "1rem",
                  textAlign: "center",
                }}
              >
            ...
            {!alertPrompt && quantityPrompt && (
              ...
              >
                <h3 style={{ margin: "0 0 1rem 0" }}>
                  ¿Cuántos "{quantityPrompt.item.name}"?
                </h3>
                <input
                  ref={qtyInputRef}
                  type="number"
            ...
            <div
              style={{
                ...
                gap: "1rem",
                opacity: quantityPrompt || alertPrompt ? 0.3 : 1,
                pointerEvents: quantityPrompt || alertPrompt ? "none" : "auto",
              }}
            >
            ...
            >
              <div style={{ textAlign: "center" }}>
                ...
                <p style={{ margin: 0, color: "var(--text-muted)" }}>
                  Ingresa el RUT del cliente para el comprobante
                </p>
              </div>
          ...
          >
            ...
            >
              ...
              >
                ...
                >
                  Usa las flechas ← → o Tab para cambiar rápidamente
                </p>
              </div>
