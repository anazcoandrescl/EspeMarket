    ...
    reader.onload = (evt) => {
      try {
        ...
        const newProducts = data.map((row) => {
          const catName =
            row["Categoria"] || row["Categoría"] || row["Category"] || "";
          ...
          return {
            ...
            sku: row["SKU"] || row["sku"] || "",
            name:
              row["Nombre"] || row["Producto"] || row["name"] || "Sin nombre",
            cost: parseFloat(row["Costo"] || row["Precio"] || row["cost"]) || 0,
            sellPrice:
              parseFloat(
                row["Precio Venta"] || row["PrecioVenta"] || row["Venta"],
              ) || 0,
            unit: row["Unidad"] || row["Medida"] || row["unit"] || "un",
            category: catName,
            stock:
              parseFloat(row["Stock"] || row["stock"] || row["Cantidad"]) || 0,
          };
        });
        ...
        setProducts((prev) => {
          ...
          newProducts.forEach((np) => {
            ...
            if (existing >= 0) {
              if (
                merged[existing].cost > 0 &&
                np.cost > merged[existing].cost * 1.05
              ) {
                const increase = (
                  (np.cost / merged[existing].cost - 1) *
                  100
                ).toFixed(1);
                newAlerts.push(
                  `El costo de "${np.name}" subió un ${increase}% (de ${formatCLP(merged[existing].cost)} a ${formatCLP(np.cost)}). Considera ajustar su precio de venta.`,
                );
              }
              merged[existing] = {
                ...merged[existing],
                sku: np.sku || merged[existing].sku || "",
                cost: np.cost,
                sellPrice: np.sellPrice,
                unit: np.unit,
                category: np.category,
                stock: (merged[existing].stock || 0) + (np.stock || 0),
              };
            } else {
        ...
        alert(
          'Hubo un error al leer el archivo Excel. Asegúrate de que tenga columnas como "Nombre", "Costo", "Precio Venta", "Unidad".',
        );
      } finally {
        setLoading(false);
          ...
          <div style={{ padding: "2rem" }}>
            ...
            >
              ...
              >
                ...
                >
                  <span>Costo:</span> <strong>{formatCLP(cost)}</strong>
                </div>
                <div
                  style={{
                    display: "flex",
                ...
                >
                  <span>Venta:</span> <strong>{formatCLP(sellPrice)}</strong>
                </div>
                <div
                  style={{
                    display: "flex",
                ...
                >
                  ...
                  <strong
                    style={{ color: profit >= 0 ? "#10B981" : "var(--danger)" }}
                  >
                    {formatCLP(profit)}
                  </strong>
                </div>
            ...
            >
              Proyección de Inventario Actual ({stock} {infoProduct.unit})
            </h4>
            <div
              style={{
    ...
    <div>
      ...
      >
        <p style={{ color: "var(--text-muted)", marginBottom: "1rem" }}>
          Sube un archivo Excel con las columnas <strong>Nombre</strong>,{" "}
          <strong>Categoria</strong>, <strong>Costo</strong>,{" "}
          <strong>Precio Venta</strong>, <strong>Unidad</strong> y{" "}
          <strong>Stock</strong>. Los productos existentes sumarán el stock y
          actualizarán su precio.
        </p>
                  ...
                  return (
                    <tr key={product.id}>
                      ...
                      <td>
                        <span
                          className="badge"
                          style={{ background: "var(--panel-alt-2)" }}
                        >
                          {product.unit}
                        </span>
                      </td>
