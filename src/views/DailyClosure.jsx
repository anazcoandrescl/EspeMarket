...
const DailyClosure = ({ sales, products, settings }) => {
  ...
  const todaySales = useMemo(
    () =>
      sales.filter(
        (s) => new Date(s.date).toLocaleDateString("es-CL") === todayStr,
      ),
    [sales, todayStr],
  );
  ...
  const totalProfit = todaySales.reduce(
    (a, s) => a + (Number(s.profit) || 0),
    0,
  );
  const totalDiscount = todaySales.reduce(
    (a, s) => a + (Number(s.discountApplied) || 0),
    0,
  );
  const margin =
    ...
    <div>
      ...
      {todaySales.length === 0 ? (
        ...
        >
          ...
          <p>
            Realiza tu primera venta desde el Punto de Venta para ver el cierre
            aquí.
          </p>
        </div>
      ) : (
        <div ref={printRef}>
          ...
          >
            ...
            <p style={{ color: "#666", fontSize: "0.9rem" }}>
              Cierre de Caja del {todayStr} — Generado a las{" "}
          ...
          >
            <div>
              ...
              <p style={{ color: "var(--text-muted)", fontSize: "0.85rem" }}>
                {todaySales.length} transacción
                {todaySales.length !== 1 ? "es" : ""} realizadas
              </p>
            </div>
          ...
          >
            <div
              className="glass-panel"
              style={{ padding: "1.5rem", borderTop: "4px solid #f59e0b" }}
            >
              <p
                style={{
                  color: "var(--text-muted)",
              ...
              >
                <DollarSign size={14} /> Descuentos Aplicados
              </p>
              <h3 style={{ fontSize: "1.5rem", color: "#f59e0b" }}>
                {formatCLP(totalDiscount)}
              </h3>
            </div>
            <div
              className="glass-panel"
              style={{ padding: "1.5rem", borderTop: "4px solid #a855f7" }}
            >
              <p
                style={{
                  color: "var(--text-muted)",
              ...
              >
                <Award size={14} /> Producto Estrella
              </p>
              ...
              {topProduct && (
                <p style={{ fontSize: "0.8rem", color: "var(--text-muted)" }}>
                  {topProduct[1]} unidades vendidas
                </p>
              )}
            </div>
            <div
              className="glass-panel"
              style={{ padding: "1.5rem", borderTop: "4px solid #ec4899" }}
            >
              <p
                style={{
                  color: "var(--text-muted)",
              ...
              >
                <Package size={14} /> Alertas de Stock
              </p>
              <h3
                style={{
                  fontSize: "1.5rem",
          ...
          >
            ...
            >
              <CreditCard size={18} color="var(--primary)" /> Desglose por
              Método de Pago
            </h2>
          ...
          >
            ...
            >
              <TrendingUp size={18} color="var(--primary)" /> Transacciones del
              Día ({todaySales.length})
            </h2>
            <table className="glass-table">
              <thead>
                <tr>
                ...
                {[...todaySales]
                  ...
                  .map((s) => (
                    <tr key={s.id}>
                      ...
                      >
                        {new Date(s.date).toLocaleTimeString("es-CL", {
                          hour: "2-digit",
                          minute: "2-digit",
                        })}
                      </td>
                      ...
                      <td style={{ textAlign: "right", fontWeight: 600 }}>
                        {formatCLP(Number(s.revenue) || 0)}
                      </td>
                      <td style={{ textAlign: "right", color: "#10b981" }}>
                        {formatCLP(Number(s.profit) || 0)}
                      </td>
                      <td>
                        <span
                          style={{
          ...
          >
            {settings?.businessName || "EspeMarket"} · Cierre del {todayStr} ·
            Sistema EspeMarket
          </div>
        </div>
      )}
