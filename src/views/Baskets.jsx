...
import { Plus, Trash2, Edit2, Save, X, ShoppingBasket, DollarSign, Eye, EyeOff, Layers, Copy, FileText, CheckSquare, CheckCircle, TrendingUp, Info, PieChart } from 'lucide-react';
...
const Baskets = ({ products, setProducts, baskets, setBaskets, settings, sales, setSales }) => {
  ...
  const deleteBasket = (id) => {
    if (window.confirm('¿Eliminar esta canasta? (Su stock armado será desarmado y devuelto al inventario de productos)')) {
      const basketToDelete = baskets.find(b => b.id === id);
      if (basketToDelete && basketToDelete.stock > 0) {
        // Return products to inventory
        setProducts(prevProducts => prevProducts.map(p => {
          const itemInBasket = basketToDelete.items.find(i => i.productId === p.id);
          if (itemInBasket) {
            return { ...p, stock: (p.stock || 0) + (itemInBasket.quantity * basketToDelete.stock) };
          }
          return p;
        }));
      }
  ...
  const updateStock = (id, newStock) => {
    ...
    if (diff > 0) {
      ...
      basket.items.forEach(item => {
        ...
        if (!prod || (prod.stock || 0) < needed) {
          canBuild = false;
          missingProducts.push(`${item.name} (Faltan ${needed - (prod?.stock || 0)})`);
        }
      });

      if (!canBuild) {
        alert(`No hay suficiente stock de productos para armar esta cantidad:\n- ${missingProducts.join('\n- ')}`);
        return;
      }
    }
    ...
    setBaskets(baskets.map(b => b.id === id ? { ...b, stock: newStock } : b));
  };
  ...
  const sellBasket = (basket) => {
    if ((basket.stock || 0) <= 0) {
      alert(`Esta canasta no tiene stock. Debes armar al menos 1 unidad (presiona +) antes de venderla.`);
      return;
    }

    setBaskets(baskets.map(b => b.id === basket.id ? { ...b, stock: basket.stock - 1 } : b));
    ...
    const newSale = {
      ...
      items: [{ id: basket.id, type: 'basket', quantity: 1, name: basket.name, sellPrice: basket.sellPrice, cost: basket.cost }]
    };
  ...
  const exportQuote = async (basket) => {
    ...
    quoteDiv.style.fontFamily = '"Inter", "Helvetica Neue", Helvetica, Arial, sans-serif';
    ...
    try {
      const canvas = await html2canvas(quoteDiv, { scale: 2, backgroundColor: '#ffffff' });
  ...
  const addItemToBasket = (productId) => {
    ...
    const existingItemIndex = currentBasket.items.findIndex(i => i.productId === product.id);
    ...
    recalculateBasket(newItems, currentBasket.sellPrice, currentBasket.extraCosts);
  };
  ...
  const removeItem = (productId) => {
    const newItems = currentBasket.items.filter(i => i.productId !== productId);
    recalculateBasket(newItems, currentBasket.sellPrice, currentBasket.extraCosts);
  };

  const recalculateBasket = (items, sellPrice, extraCosts) => {
    const itemsCost = items.reduce((acc, item) => acc + (item.cost * parseQuantity(item.quantity)), 0);
    const totalCost = itemsCost + (parseFloat(extraCosts) || 0);
    const suggestedPrice = items.reduce((acc, item) => acc + ((item.sellPrice || 0) * parseQuantity(item.quantity)), 0) + (parseFloat(extraCosts) || 0);
  ...
  if (editingId !== null) {
    ...
    const margin = currentBasket.sellPrice > 0 ? (profit / currentBasket.sellPrice) * 100 : 0;
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 350px', gap: '2rem' }}>
          <div className="glass-panel" style={{ padding: '2rem' }}>
            <div style={{ marginBottom: '2rem' }}>
              <label style={{ display: 'block', marginBottom: '0.5rem', color: 'var(--text-muted)' }}>Nombre de la Canasta</label>
              <input 
                className="glass-input" 
                value={currentBasket.name}
                onChange={(e) => setCurrentBasket({...currentBasket, name: e.target.value})}
            ...
            <div style={{ marginBottom: '1.5rem', display: 'flex', gap: '1rem', alignItems: 'flex-end' }}>
              <div style={{ flex: 1 }}>
                <label style={{ display: 'block', marginBottom: '0.5rem', color: 'var(--text-muted)' }}>Agregar Producto</label>
                <select 
                  className="glass-input"
                  onChange={(e) => {
                ...
                >
                  ...
                  {products.map(p => (
                    <option key={p.id} value={p.id}>{p.name} ({formatCLP(p.cost)}/{p.unit})</option>
                  ))}
                </select>
              </div>
            </div>
            ...
            {currentBasket.items.length > 0 ? (
              <div className="table-container" style={{ border: 'none', background: 'var(--panel-alt)' }}>
                  ...
                  <tbody>
                    {currentBasket.items.map(item => (
                      <tr key={item.productId}>
                        <td>{item.name} <span style={{ fontSize: '0.8rem', color: 'var(--text-muted)' }}>({item.unit})</span></td>
                        ...
                        <td>{formatCLP(item.cost * parseQuantity(item.quantity))}</td>
              ...
              <div style={{ textAlign: 'center', padding: '3rem', background: 'rgba(0,0,0,0.1)', borderRadius: '8px', color: 'var(--text-muted)' }}>
          ...
          <div className="glass-panel" style={{ padding: '2rem', height: 'fit-content' }}>
            ...
            <div style={{ marginBottom: '1.5rem', paddingBottom: '1.5rem', borderBottom: '1px dashed var(--surface-border)' }}>
              <label style={{ display: 'block', marginBottom: '0.5rem', color: 'var(--text-muted)' }}>Costos Extra (Packaging, Envío)</label>
              <div style={{ position: 'relative' }}>
                ...
                <input 
                  ...
                  onChange={(e) => recalculateBasket(currentBasket.items, currentBasket.sellPrice, e.target.value)}
                  placeholder="0"
                />
              </div>
            </div>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '0.5rem' }}>
              ...
              <span style={{ fontWeight: 'bold', fontSize: '1.1rem' }}>{formatCLP(currentBasket.cost)}</span>
            </div>
            
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '1rem', paddingBottom: '1rem', borderBottom: '1px solid var(--surface-border)' }}>
              <span style={{ color: 'var(--text-muted)' }}>Precio Sugerido:</span>
              <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                <span style={{ fontWeight: 'bold', color: 'var(--text-muted)' }}>{formatCLP(currentBasket.suggestedPrice || 0)}</span>
                <button 
                  onClick={() => recalculateBasket(currentBasket.items, currentBasket.suggestedPrice, currentBasket.extraCosts)}
                  style={{ background: 'var(--primary)', border: 'none', color: 'white', borderRadius: '4px', padding: '0.2rem 0.5rem', cursor: 'pointer', fontSize: '0.75rem' }}
            ...
            <div style={{ marginBottom: '1.5rem' }}>
              ...
              <div style={{ position: 'relative' }}>
                ...
                <input 
                  type="number" 
                  className="glass-input" 
                  style={{ paddingLeft: '2rem', fontSize: '1.25rem', fontWeight: 'bold', color: '#10B981' }}
                  value={currentBasket.sellPrice}
                  onChange={(e) => recalculateBasket(currentBasket.items, e.target.value, currentBasket.extraCosts)}
                />
              </div>
            </div>

            <div style={{ background: 'var(--panel-alt)', padding: '1rem', borderRadius: '8px' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '0.5rem' }}>
                <span style={{ color: 'var(--text-muted)' }}>Ganancia Neta:</span>
                <span style={{ fontWeight: 'bold', color: profit >= 0 ? '#10B981' : 'var(--danger)' }}>
                  {formatCLP(profit)}
                </span>
              </div>
              <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                <span style={{ color: 'var(--text-muted)' }}>Margen:</span>
                <span className={`badge ${margin > 30 ? 'success' : ''}`} style={{ background: margin > 30 ? 'rgba(16, 185, 129, 0.2)' : 'rgba(255,255,255,0.1)' }}>
                  {margin.toFixed(1)}%
                </span>
              </div>
            </div>
      ...
      <div className="grid-cards">
        {baskets.length === 0 ? (
          <div className="glass-panel" style={{ gridColumn: '1 / -1', padding: '4rem', textAlign: 'center', color: 'var(--text-muted)' }}>
            <ShoppingBasket size={48} style={{ margin: '0 auto 1rem', opacity: 0.5 }} />
            <p>Aún no has armado ninguna canasta.</p>
          </div>
        ) : (
          baskets.map(basket => {
            const profit = basket.sellPrice - basket.cost;
            const margin = basket.sellPrice > 0 ? (profit / basket.sellPrice) * 100 : 0;
            
            return (
              <div key={basket.id} className="glass-panel" style={{ display: 'flex', flexDirection: 'column' }}>
                <div style={{ padding: '1.5rem', borderBottom: '1px solid var(--surface-border)' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '0.5rem' }}>
                    <h3 style={{ fontSize: '1.25rem', margin: 0 }}>{basket.name}</h3>
                    <div style={{ display: 'flex', alignItems: 'center', background: 'var(--panel-alt)', borderRadius: '8px', border: '1px solid var(--surface-border)' }} title="Stock Disponible">
                      <button onClick={() => updateStock(basket.id, (basket.stock || 0) - 1)} style={{ background:'none', border:'none', color:'var(--text-main)', cursor:'pointer', padding:'0.25rem 0.5rem', borderRadius: '8px 0 0 8px' }}>-</button>
                      <span style={{ fontSize: '0.9rem', fontWeight: 'bold', width: '24px', textAlign: 'center', color: (basket.stock || 0) > 0 ? '#10B981' : 'var(--text-muted)' }}>{basket.stock || 0}</span>
                      <button onClick={() => updateStock(basket.id, (basket.stock || 0) + 1)} style={{ background:'none', border:'none', color:'var(--text-main)', cursor:'pointer', padding:'0.25rem 0.5rem', borderRadius: '0 8px 8px 0' }}>+</button>
                    </div>
                  </div>
                ...
                <div style={{ padding: '1.5rem', flex: 1 }}>
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '0.5rem' }}>
                    ...
                    <span style={{ fontWeight: 'bold' }}>{formatCLP(basket.sellPrice)}</span>
                  </div>
                  <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: '1rem', paddingTop: '1rem', borderTop: '1px dashed var(--surface-border)' }}>
                ...
                {checklistId === basket.id ? (
                  <div style={{ padding: '1.5rem', background: 'rgba(59, 130, 246, 0.1)', borderTop: '1px solid var(--surface-border)' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1rem' }}>
                      ...
                      <button onClick={() => toggleChecklist(basket.id)} style={{ background: 'none', border: 'none', color: 'var(--text-muted)', cursor: 'pointer' }}><X size={16}/></button>
                    </div>
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
                      {basket.items.map((item, idx) => (
                        <label key={idx} style={{ display: 'flex', alignItems: 'center', gap: '1rem', padding: '0.75rem', background: checkedItems[idx] ? 'rgba(16, 185, 129, 0.1)' : 'var(--panel-alt)', borderRadius: '8px', cursor: 'pointer', border: checkedItems[idx] ? '1px solid #10B981' : '1px solid transparent', transition: 'all 0.2s' }}>
                          <input 
                            type="checkbox" 
                          ...
                          />
                          <span style={{ fontSize: '1.1rem', fontWeight: checkedItems[idx] ? 'normal' : 'bold', color: checkedItems[idx] ? 'var(--text-muted)' : 'var(--text-main)', textDecoration: checkedItems[idx] ? 'line-through' : 'none' }}>
                            {item.quantity}x {item.name} <span style={{ fontSize: '0.8rem', opacity: 0.7 }}>({item.unit})</span>
                          </span>
                        </label>
                      ))}
                    </div>
                  ...
                  <div style={{ padding: '1.5rem', background: 'var(--panel-alt-2)', borderTop: '1px solid var(--surface-border)' }}>
                    <h4 style={{ marginBottom: '0.5rem', fontSize: '0.9rem', color: 'var(--text-muted)' }}>Contenido:</h4>
                    <ul style={{ listStyle: 'none', padding: 0, margin: 0, fontSize: '0.85rem' }}>
                      {basket.items.map((item, idx) => (
                        <li key={idx} style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '0.25rem' }}>
                          <span>{item.quantity}x {item.name} <span style={{ color: 'var(--text-muted)' }}>({item.unit})</span></span>
                          <span>{formatCLP(item.cost * parseQuantity(item.quantity))}</span>
                        </li>
                      ))}
                    </ul>
                  </div>
                ...
                <div style={{ padding: '1rem', background: 'var(--panel-alt)', borderTop: '1px solid var(--surface-border)' }}>
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.5rem', marginBottom: '0.5rem' }}>
                    <button 
                      className="glass-button secondary" 
                      style={{ padding: '0.75rem', fontSize: '0.9rem', color: '#10B981', border: '1px solid rgba(16, 185, 129, 0.3)' }}
                    ...
                    <button 
                      className="glass-button secondary" 
                      style={{ padding: '0.75rem', fontSize: '0.9rem', color: '#60a5fa', border: '1px solid rgba(59, 130, 246, 0.3)' }}
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5, 1fr)', gap: '0.5rem' }}>
                    ...
                    <button 
                      className="glass-button secondary" 
                      style={{ padding: '0.5rem', fontSize: '0.85rem', color: 'var(--primary)' }}
        ...
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.7)', backdropFilter: 'blur(4px)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000, padding: '1rem' }}>
          <div className="glass-panel" style={{ width: '100%', maxWidth: '600px', maxHeight: '90vh', overflowY: 'auto' }}>
            <div style={{ padding: '1.5rem', borderBottom: '1px solid var(--surface-border)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              ...
              <button onClick={() => setInfoBasket(null)} style={{ background: 'none', border: 'none', color: 'var(--text-muted)', cursor: 'pointer' }}>
                <X size={24} />
              </button>
            </div>
            
            <div style={{ padding: '2rem' }}>
              <h3 style={{ fontSize: '1.5rem', marginBottom: '0.5rem' }}>{infoBasket.name}</h3>
              <p style={{ color: 'var(--text-muted)', marginBottom: '1.5rem' }}>Stock actual armado: {infoBasket.stock || 0} unidades</p>
              ...
              <div className="grid-cards" style={{ gridTemplateColumns: '1fr 1fr', gap: '1.5rem', marginBottom: '2rem' }}>
                <div style={{ background: 'var(--panel-alt)', padding: '1.5rem', borderRadius: '12px', borderLeft: '4px solid var(--primary)' }}>
                  <p style={{ color: 'var(--text-muted)', fontSize: '0.9rem', marginBottom: '0.5rem', display: 'flex', alignItems: 'center', gap: '0.5rem' }}><DollarSign size={14}/> Por Canasta</p>
                  <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '0.5rem' }}><span>Costo:</span> <strong>{formatCLP(infoBasket.cost)}</strong></div>
                  <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '0.5rem' }}><span>Venta:</span> <strong>{formatCLP(infoBasket.sellPrice)}</strong></div>
                  <div style={{ display: 'flex', justifyContent: 'space-between', borderTop: '1px solid var(--surface-border)', paddingTop: '0.5rem', marginTop: '0.5rem' }}>
                    <span>Ganancia Neta:</span> <strong style={{ color: (infoBasket.sellPrice - infoBasket.cost) >= 0 ? '#10B981' : 'var(--danger)' }}>{formatCLP(infoBasket.sellPrice - infoBasket.cost)}</strong>
                  </div>
                </div>
                ...
                <div style={{ background: 'var(--panel-alt)', padding: '1.5rem', borderRadius: '12px', borderLeft: '4px solid #f59e0b' }}>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', height: '100%' }}>
                    <h3 style={{ fontSize: '2.5rem', margin: 0, color: (infoBasket.sellPrice > 0 ? ((infoBasket.sellPrice - infoBasket.cost) / infoBasket.sellPrice) * 100 : 0) >= 30 ? '#10B981' : ((infoBasket.sellPrice > 0 ? ((infoBasket.sellPrice - infoBasket.cost) / infoBasket.sellPrice) * 100 : 0) > 0 ? '#f59e0b' : 'var(--danger)') }}>
                      {(infoBasket.sellPrice > 0 ? ((infoBasket.sellPrice - infoBasket.cost) / infoBasket.sellPrice) * 100 : 0).toFixed(1)}%
                    </h3>
              ...
              <h4 style={{ marginBottom: '1rem', borderBottom: '1px solid var(--surface-border)', paddingBottom: '0.5rem' }}>Proyección de Inventario Armado ({(infoBasket.stock || 0)} unidades)</h4>
              <div style={{ background: 'rgba(16, 185, 129, 0.1)', border: '1px solid rgba(16, 185, 129, 0.3)', padding: '1.5rem', borderRadius: '12px' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '0.5rem' }}>
                  <span style={{ color: 'var(--text-muted)' }}>Capital Invertido en Armado:</span>
                  <span>{formatCLP(infoBasket.cost * (infoBasket.stock || 0))}</span>
                </div>
                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '0.5rem' }}>
                  <span style={{ color: 'var(--text-muted)' }}>Ingreso Bruto Estimado:</span>
                  <span>{formatCLP(infoBasket.sellPrice * (infoBasket.stock || 0))}</span>
                </div>
                <div style={{ display: 'flex', justifyContent: 'space-between', borderTop: '1px solid rgba(255,255,255,0.1)', paddingTop: '0.5rem', marginTop: '0.5rem' }}>
                  ...
                  <strong style={{ fontSize: '1.2rem', color: '#10B981' }}>{formatCLP((infoBasket.sellPrice - infoBasket.cost) * (infoBasket.stock || 0))}</strong>
                </div>
              </div>
              
              <div style={{ marginTop: '2rem', textAlign: 'center' }}>
                <button onClick={() => setInfoBasket(null)} className="glass-button" style={{ display: 'inline-flex', padding: '0.75rem 2rem' }}>
                  Cerrar Detalles
                </button>
              </div>
            </div>
