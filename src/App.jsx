...
function App() {
  const [accessRole, setAccessRole] = useLocalStorage('canasta_access_role', null); // 'admin', 'pos', or null
  const [activeTab, setActiveTab] = useLocalStorage('canasta_active_tab', 'dashboard');
  ...
  useEffect(() => {
    ...
    if (theme === 'auto') {
      ...
      const listener = (e) => root.setAttribute('data-theme', e.matches ? 'light' : 'dark');
  ...
  const renderContent = () => {
    switch (activeTab) {
      case 'dashboard':
        return <Dashboard products={products} baskets={baskets} sales={sales} />;
      case 'live':
      ...
      case 'settings':
        return <Settings settings={settings} setSettings={setSettings} products={products} setProducts={setProducts} baskets={baskets} setBaskets={setBaskets} sales={sales} setSales={setSales} categories={categories} setCategories={setCategories} />;
      default:
        return <Dashboard products={products} baskets={baskets} sales={sales} />;
    }
  };
  ...
  const handleAdminLogin = async (e) => {
    ...
    const email = adminUser.includes('@') ? adminUser : `${adminUser.toLowerCase()}@espemarket.com`;
          ...
          <div className="glass-panel" style={{ padding: '3rem', maxWidth: '500px', width: '100%', textAlign: 'center', position: 'relative' }}>
            {showAdminLogin ? (
              <form onSubmit={handleAdminLogin} style={{ display: 'flex', flexDirection: 'column', gap: '1rem', textAlign: 'left' }}>
                <h2 style={{ textAlign: 'center', marginBottom: '1.5rem' }}>Acceso Administrativo</h2>
                ...
                <div style={{ display: 'flex', gap: '1rem', marginTop: '1rem' }}>
                  <button type="button" className="glass-button secondary" onClick={() => setShowAdminLogin(false)} style={{ flex: 1, justifyContent: 'center' }}>Volver</button>
              ...
              <>
                ...
                <h1 style={{ marginBottom: '0.5rem', fontSize: '2rem' }}>{settings.businessName}</h1>
                <p style={{ color: 'var(--text-muted)', marginBottom: '3rem' }}>Selecciona tu modo de acceso</p>
  ...
  return (
    <>
      ...
      <aside className="sidebar">
        <div>
          <h2 style={{ color: 'var(--text-main)', marginBottom: '2rem', padding: '0 1rem' }}>
            🛒 EspeMarket
          </h2>
        </div>
        <nav style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
          ...
          <button 
            ...
            style={{ background: 'transparent', border: 'none', width: '100%', textAlign: 'left', color: 'var(--primary)' }}
          >
            <Store size={20} />
            Punto de Venta
          </button>
          ...
          <button 
            ...
            style={{ background: 'transparent', border: 'none', width: '100%', textAlign: 'left', color: 'var(--danger)' }}
          >
            <Tag size={20} />
            Creador de Ofertas
          </button>
          ...
          <div style={{ marginTop: 'auto', paddingTop: '2rem', borderTop: '1px solid var(--surface-border)' }}>
            ...
            <div style={{ marginTop: '1rem', paddingTop: '1rem', borderTop: '1px solid var(--surface-border)' }}>
              <p style={{ fontSize: '0.75rem', color: 'var(--text-muted)', marginBottom: '0.5rem', textAlign: 'center' }}>Apariencia</p>
              <div style={{ display: 'flex', gap: '0.25rem', background: 'rgba(0,0,0,0.2)', padding: '0.25rem', borderRadius: '8px' }}>
                <button 
                  onClick={() => setTheme('light')} 
                  style={{ flex: 1, display: 'flex', justifyContent: 'center', background: theme === 'light' ? 'var(--primary)' : 'transparent', border: 'none', padding: '0.5rem', borderRadius: '6px', color: theme === 'light' ? 'white' : 'var(--text-muted)', cursor: 'pointer', transition: 'all 0.2s' }}
                ...
                <button 
                  ...
                  style={{ flex: 1, display: 'flex', justifyContent: 'center', background: theme === 'dark' ? 'var(--primary)' : 'transparent', border: 'none', padding: '0.5rem', borderRadius: '6px', color: theme === 'dark' ? 'white' : 'var(--text-muted)', cursor: 'pointer', transition: 'all 0.2s' }}
                ...
                <button 
                  ...
                  style={{ flex: 1, display: 'flex', justifyContent: 'center', background: theme === 'auto' ? 'var(--primary)' : 'transparent', border: 'none', padding: '0.5rem', borderRadius: '6px', color: theme === 'auto' ? 'white' : 'var(--text-muted)', cursor: 'pointer', transition: 'all 0.2s' }}
            ...
            <button 
              ...
              className="glass-button secondary"
              style={{ width: '100%', marginTop: '1rem', justifyContent: 'center', borderColor: 'var(--danger)', color: 'var(--danger)' }}
            >
              Cerrar Sesión
            </button>
