...
const SupabaseSync = ({ inline = false }) => {
  ...
  const syncToSupabase = useCallback(
    async (isRetry = false) => {
      try {
        ...
        st._offers_db = JSON.parse(
          localStorage.getItem("canasta_offers") || "[]",
        );

        const payload = {
          products: JSON.parse(
            localStorage.getItem("canasta_products") || "[]",
          ),
          baskets: JSON.parse(localStorage.getItem("canasta_baskets") || "[]"),
          sales: JSON.parse(localStorage.getItem("canasta_sales") || "[]"),
          categories: JSON.parse(
            localStorage.getItem("canasta_categories") || "[]",
          ),
          settings: st,
        };
        ...
        if (currentRetry < MAX_RETRIES) {
          ...
          console.log(
            `Reintentando en ${delay / 1000}s (intento ${currentRetry}/${MAX_RETRIES})...`,
          );
  ...
  useEffect(() => {
    ...
    channelRef.current
      .on("broadcast", { event: "db-update" }, (payloadMessage) => {
        isSyncingFromRemote.current = true;
  ...
  if (isMobile) {
    ...
    return (
      ...
      >
        {status === "connecting" && (
          <RefreshCw size={16} className="spin" color="var(--text-muted)" />
        )}
        {status === "syncing" && (
          <RefreshCw size={16} className="spin" color={iconColor} />
        )}
  ...
  return (
    <div style={style} title={lastError ? `Último error: ${lastError}` : ""}>
      {status === "connecting" && (
        <>
          <RefreshCw size={14} className="spin" color="var(--text-muted)" />{" "}
      ...
      {status === "syncing" && (
        <>
          ...
          <span style={{ color: "var(--primary)" }}>Sincronizando...</span>
        </>
      )}
      {status === "synced" && (
        <>
          <Cloud size={14} color="#10B981" />{" "}
          <span style={{ color: "#10B981" }}>Sincronizado en la nube</span>
        </>
      )}
      {status === "error" && (
        <>
